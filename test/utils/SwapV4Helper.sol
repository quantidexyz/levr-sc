...
import {Currency, CurrencyLibrary} from '@uniswapV4-core/types/Currency.sol';
import {IPoolManager} from '@uniswapV4-core/interfaces/IPoolManager.sol';
import {StateLibrary} from '@uniswapV4-core/libraries/StateLibrary.sol';
import {IPermit2} from '@uniswapV4-permit2/interfaces/IPermit2.sol';

// Universal Router imports
import {IUniversalRouter} from '@uniswapV4-universal-router/contracts/interfaces/IUniversalRouter.sol';
import {Commands} from '@uniswapV4-universal-router/contracts/libraries/Commands.sol';

// V4 Periphery imports
import {IV4Router} from '@uniswapV4-universal-router/lib/v4-periphery/src/interfaces/IV4Router.sol';
import {Actions} from '@uniswapV4-universal-router/lib/v4-periphery/src/libraries/Actions.sol';
...
contract SwapV4Helper is Test {
    ...
    IPoolManager public constant POOL_MANAGER =
        IPoolManager(0x498581fF718922c3f8e6A244956aF099B2652b2b); // Base V4 Pool Manager
    ...
    function executeSwap(SwapParams memory params) external payable returns (uint256 amountOut) {
        ...
        bool isInputNative = Currency.unwrap(inputCurrency) == address(0) ||
            Currency.unwrap(inputCurrency) == WETH;
        bool isOutputNative = Currency.unwrap(outputCurrency) == address(0) ||
            Currency.unwrap(outputCurrency) == WETH;
    ...
    function _setupTokenApprovals(Currency currency, uint256 amount) internal {
        ...
        (uint160 allowance, uint48 expiration, ) = PERMIT2.allowance(
            address(this),
            token,
    ...
    function checkSwapRequirements(
        ...
        if (token == address(0) || token == WETH) {
            ...
            (uint160 routerAllowance, uint48 expiration, ) = PERMIT2.allowance(
                user,
                token,
                address(UNIVERSAL_ROUTER)
            );
