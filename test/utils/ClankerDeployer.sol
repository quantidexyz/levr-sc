...
contract ClankerDeployer {
  ...
  address internal constant AIRDROP_EXTENSION_DEFAULT = 0xf652B3610D75D81871bf96DB50825d9af28391E0;
  ...
  function _staticFeeData(uint24 clankerFeeBps, uint24 pairedFeeBps) internal pure returns (bytes memory) {
  ...
  function deployFactoryStaticFullWithOptions(
    ...
    bytes memory lockerData = abi.encode(ClankerLpLockerInstantiation({feePreference: feePreferences}));
    ...
    if (enableAirdrop) {
      ...
      if (extData.length == 0) {
        ...
        extData = abi.encode(opts.admin, opts.merkleRoot, opts.lockupDuration, opts.vestingDuration);
      }
