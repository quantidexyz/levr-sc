...
/// @title IMasterLevr_v1 - Interface for the Levr protocol's MasterLevr contract
...
interface IMasterLevr_v1 {
    ...
    event Staked(uint256 indexed levrId, address indexed user, uint256 amount);
    ...
    /// @return levrId Unique identifier for this pool
    ...
    /// @param levrId The pool identifier
    ...
    /// @notice Get the levrId for a given underlying token
    /// @param underlying The underlying token address
    /// @return levrId The pool identifier (0 if not registered)
    function getLevrIdByUnderlying(
        address underlying
    ) external view returns (uint256 levrId);
}
